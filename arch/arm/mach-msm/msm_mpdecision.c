...
#define MSM_MPDEC_IDLE_FREQ             432000
#define MSM_MPDEC_SCROFF_FREQ		432000
...
static void msm_mpdec_early_suspend(struct early_suspend *h)
{
	...
	for_each_possible_cpu(cpu) {
		...
		if ((cpu_online(cpu) = 1) && (msm_prdec_tuners_ins.scroff_profile)) {
...
static ssize_t show_mpdec_scroff_freq (struct kobject *kobj, struct attribute *attr, char *buf)
{
...
static ssize_t store_mpdec_scroff_freq(struct kobject *a, struct attribute *b, const char *buf, size_t count)
{
	long unsigned int input;
	int ret;
...
static ssize_t store_scroff_profile(struct kobject *a, struct attribute *b, const char *buf, size_t count)
{
	unsigned int input;
	size_t ret = count;
	int cpu;
